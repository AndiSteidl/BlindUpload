            ...
            for file in files:
                if file and allowed_file(file.filename):
                    try:
                        ...
                        # PRESERVE EXACT ORIGINAL EXTENSION & BYTES (HEIC, JPG, PNG, MP4, MOV, etc.)
                        ...
                        # 3. Offload thumbnail creation to background executor
...
def upload_chunk():
    ...
    Accepts 20 MB slices of large files (videos > 20 MB) to effortlessly bypass Cloudflare Free's 100 MB limit.
    """
    user_id = ensure_session()
    ...
    if not file_id or chunk_index is None or total_chunks is None or not original_filename or not chunk_file:
        return jsonify({'error': 'Ungültige Chunk-Parameter.'}), 400
